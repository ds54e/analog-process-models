...
from apm.research_numerics import (
    ResearchError,
    aggregate_tail_risk,
    canonical_hash,
...
def test_extraction_endpoint():
    ...
    with pytest.raises(ResearchError, match='ENDPOINT'):
        extract_mg(u, np.exp(u) * 1e-06)

def test_extraction_duplicate_axis():
    with pytest.raises(ResearchError, match='AXIS'):
