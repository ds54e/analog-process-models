...
BENCHMARK_MODES = ("process", "mismatch", "all")
BENCHMARK_CORNERS = ("bench_tt", "bench_ff", "bench_ss", "bench_fs", "bench_sf")
REQUEST_SCHEMA = "apm.benchmark-request.v1"
RESOLVED_SCHEMA = "apm.resolved-variation.v1"
...
def load_benchmark_configuration(root: Path | None = None) -> dict[str, Any]:
    ...
    paths = {
        "variation": selected / "variation/benchmark_v1.toml",
        "passives": selected / "passives/benchmark_v1.toml",
        "adapters": selected / "variation/adapters_v1.toml",
    }
...
def _normalize_request(
    ...
    adapters = configuration["adapters"]["kit"]
    ...
    for raw in raw_mos:
        ...
        if kit_id not in adapters:
            raise BenchmarkError(f"no calibrated benchmark adapter for kit: {kit_id}")
        polarity = raw.get("polarity")
        if polarity not in ("n", "p"):
            raise BenchmarkError(f"MOS {instance_id} polarity must be 'n' or 'p'")
        geometry = raw.get("geometry")
        if not isinstance(geometry, dict):
            raise BenchmarkError(f"MOS {instance_id} geometry must be an object")
        adapter = adapters[kit_id]
        ...
        if adapter["architecture"] == "planar_bulk":
            ...
            match_size = (w_m * l_m) / (
                float(adapter["reference_w_m"]) * float(adapter["reference_l_m"])
            )
        elif adapter["architecture"] == "finfet":
            ...
            match_size = (nfin * l_m) / (
                int(adapter["reference_nfin"]) * float(adapter["reference_l_m"])
            )
        else:
            raise BenchmarkError(f"unsupported adapter architecture for {kit_id}")
...
def _invert_observable_fit(target: float, linear: float, quadratic: float) -> float:
    ...
    candidates = ((-linear + root) / (2.0 * quadratic), (-linear - root) / (2.0 * quadratic))
...
def _raw_adapter(
    ...
    vth_raw = _invert_observable_fit(
        vth_shift_v,
        float(polarity_adapter["vth_fit_linear"]),
        float(polarity_adapter["vth_fit_quadratic"]),
    )
    drive_delta_raw = _invert_observable_fit(
        drive_shift_fraction,
        float(polarity_adapter["drive_fit_linear"]),
        float(polarity_adapter["drive_fit_quadratic"]),
    )
    drive_raw = float(adapter["drive_raw_nominal"]) + drive_delta_raw
    if drive_raw <= 0.0:
        raise BenchmarkError(
            f"resolved {kit_id}/{polarity} raw drive multiplier is not positive: {drive_raw:g}"
        )
    vth_range = configuration["adapters"]["method"]["vth_raw_sweep"]
    drive_range = configuration["adapters"]["method"]["drive_raw_multiplier_sweep"]
    device_path = polarity_adapter["ngspice_device_path_template"].format(
...
def _resolve(
    ...
    *,
    ...
    process_active = variation_mode in ("process", "all", "corner")
    mismatch_active = variation_mode in ("mismatch", "all")
    process_vth_sigma = float(variation["mos"]["process"]["vth_shift_sigma"])
    process_drive_sigma = float(variation["mos"]["process"]["drive_shift_sigma"])
    mismatch_vth_sigma = float(variation["mos"]["mismatch"]["vth_shift_sigma_ref"])
    mismatch_drive_sigma = float(variation["mos"]["mismatch"]["drive_shift_sigma_ref"])

    global_process: dict[str, Any] = {"mos": {}}
    for polarity in ("n", "p"):
        vth_z = global_z[f"mos_{polarity}_vth"]
        drive_z = global_z[f"mos_{polarity}_drive"]
        sampled_vth = process_vth_sigma * vth_z
        sampled_drive = process_drive_sigma * drive_z
        global_process["mos"][polarity] = {
            "vth_shift_z": vth_z,
            "drive_shift_z": drive_z,
            "sampled_vth_shift_v": sampled_vth,
            "sampled_drive_shift_fraction": sampled_drive,
            "applied_vth_shift_v": sampled_vth if process_active else 0.0,
            "applied_drive_shift_fraction": sampled_drive if process_active else 0.0,
        }
    global_process["passives"] = {}
    for kind, spec_name in (("resistor", "resistor"), ("capacitor", "capacitor")):
        z_value = global_z[f"{kind}_scale"]
        sampled = float(passives[spec_name]["process_sigma"]) * z_value
        global_process["passives"][kind] = {
            "scale_z": z_value,
            "sampled_scale_fraction": sampled,
            "applied_scale_fraction": sampled if process_active else 0.0,
        }
    ...
    for instance in normalized["mos"]:
        ...
        sampled_local_vth = mismatch_vth_sigma * vth_z / math.sqrt(match_size)
        sampled_local_drive = mismatch_drive_sigma * drive_z / math.sqrt(match_size)
        local_vth = sampled_local_vth if mismatch_active else 0.0
        local_drive = sampled_local_drive if mismatch_active else 0.0
        process = global_process["mos"][instance["polarity"]]
        total_vth = process["applied_vth_shift_v"] + local_vth
        process_drive_factor = 1.0 + process["applied_drive_shift_fraction"]
        local_drive_factor = 1.0 + local_drive
        total_drive_factor = process_drive_factor * local_drive_factor
        if process_drive_factor <= 0.0 or local_drive_factor <= 0.0 or total_drive_factor <= 0.0:
            raise BenchmarkError(
    ...
    for plural, singular in (("resistors", "resistor"), ("capacitors", "capacitor")):
        process = global_process["passives"][singular]
        sigma_ref = float(passives[singular]["mismatch_sigma_ref"])
        for instance in normalized[plural]:
            ...
            local_scale = sampled_local if mismatch_active else 0.0
            process_factor = 1.0 + process["applied_scale_fraction"]
            local_factor = 1.0 + local_scale
            resolved_factor = process_factor * local_factor
            resolved_value = instance["value"] * resolved_factor
            if process_factor <= 0.0 or local_factor <= 0.0 or resolved_value <= 0.0:
                raise BenchmarkError(
    ...
    rng_identity: dict[str, Any] | None
    if seed is None:
        rng_identity = None
    else:
        rng_identity = {
...
def resolve_monte_carlo(
    ...
    *,
    ...
    global_z = {
        "mos_n_vth": draw("process.mos.n.vth_shift"),
        "mos_n_drive": draw("process.mos.n.drive_shift"),
        "mos_p_vth": draw("process.mos.p.vth_shift"),
        "mos_p_drive": draw("process.mos.p.drive_shift"),
        "resistor_scale": draw("process.passives.resistor.scale"),
        "capacitor_scale": draw("process.passives.capacitor.scale"),
    }
    local_z: dict[str, dict[str, float]] = {}
    for instance in normalized["mos"]:
        local_z[instance["id"]] = {
            "vth": draw(f"mismatch.mos.{instance['id']}.vth_shift"),
            "drive": draw(f"mismatch.mos.{instance['id']}.drive_shift"),
        }
    for kind in ("resistors", "capacitors"):
        for instance in normalized[kind]:
            local_z[instance["id"]] = {
                "scale": draw(f"mismatch.{kind[:-1]}.{instance['id']}.scale")
            }
    return _resolve(
        request,
        configuration,
...
def resolve_corner(
    ...
    global_z = {
        "mos_n_vth": float(vector["vth_n_sigma"]),
        "mos_n_drive": float(vector["drive_n_sigma"]),
        "mos_p_vth": float(vector["vth_p_sigma"]),
        "mos_p_drive": float(vector["drive_p_sigma"]),
        "resistor_scale": float(vector["resistor_scale_sigma"]),
        "capacitor_scale": float(vector["capacitor_scale_sigma"]),
    }
    local_z: dict[str, dict[str, float]] = {}
    for instance in normalized["mos"]:
        local_z[instance["id"]] = {"vth": 0.0, "drive": 0.0}
    for kind in ("resistors", "capacitors"):
        for instance in normalized[kind]:
